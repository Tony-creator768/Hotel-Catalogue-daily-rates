...
function rateSubline(row) {
  ...
  return parts.length ? parts.join(" • ") : "Size not listed";
}
...
function unitRowHtml(row, compact = false) {
  ...
  const content = `
    <div>
      ...
      <em class="row-trace-mini">${escapeHtml(tracePill(row))}</em>
    </div>
    <div class="unit-side">
      ${rateHtml(row)}
    </div>
      ...
      <article class="card hotel-card">
        ...
        <div class="card-body">
          ...
          <div class="unit-list-head">
            ...
            <span>${group.unitCount} source row${group.unitCount === 1 ? "" : "s"} shown</span>
          </div>
...
function detailTableRows(rows) {
  return rows.map(row => {
    ...
    const sourceLink = trace.rowUrl
      ? `<a class="source-link" href="${escapeHtml(trace.rowUrl)}" target="_blank" rel="noopener noreferrer">Row ${escapeHtml(trace.rowNumber)}</a>`
      : `${trace.rowNumber ? `Row ${escapeHtml(trace.rowNumber)}` : "—"}`;
    ...
    return `
      <tr>
        ...
        <td>${sourceLink}<div class="rate-meta">${escapeHtml(trace.recordId || "")}</div></td>
      </tr>
    `;
  }).join("");
}
...
function traceabilityHtml(rows) {
  const sampleRows = rows.slice(0, 8).map(row => traceFor(row));
  const sourceNames = uniqueItems(rows.map(row => traceFor(row).sourceContract).filter(Boolean));
  const sheetLinks = sampleRows.filter(trace => trace.rowUrl).slice(0, 6);

  return `
    <div class="detail-block full trace-block">
      <h4>Google Sheet traceability</h4>
      <p>Every displayed hotel, room type, rate, policy, contact, and note is read directly from the <strong>Catalogue_View</strong> tab. Use the record ID and sheet row below to audit the value in the Google Sheet created from the Excel file.</p>
      <div class="trace-grid">
        <div><span>Source contract(s)</span><strong>${escapeHtml(sourceNames.join(" | ") || "Not listed")}</strong></div>
        <div><span>Record ID range</span><strong>${escapeHtml(sampleRows.map(t => t.recordId).filter(Boolean).slice(0, 4).join(", ") || "Not listed")}${sampleRows.length > 4 ? "…" : ""}</strong></div>
        <div><span>Sheet row(s)</span><strong>${escapeHtml(sampleRows.map(t => t.rowNumber).filter(Boolean).slice(0, 8).join(", ") || "Not returned by API")}</strong></div>
        <div><span>Last updated</span><strong>${escapeHtml(sampleRows.map(t => t.lastUpdated).filter(Boolean)[0] || "Not listed")}</strong></div>
      </div>
      ${sheetLinks.length ? `<div class="links trace-links">${sheetLinks.map(trace => `<a href="${escapeHtml(trace.rowUrl)}" target="_blank" rel="noopener noreferrer">Open ${escapeHtml(trace.recordId || `row ${trace.rowNumber}`)}</a>`).join("")}</div>` : ""}
    </div>
  `;
}
  ...
  els.modalBody.innerHTML = `
    <div class="modal-card no-detail-image">
      ...
      <div class="modal-content">
        <p class="modal-eyebrow">Google Sheet traceable contract data</p>
...
function mailtoLink(group, row) {
  ...
  const body = [
    ...
    `Record ID: ${traceFor(row).recordId || "N/A"}`,
    `Sheet row: ${traceFor(row).rowNumber || "N/A"}`,
...
function compareOptionLabel(row) {
  const parts = [
    roomTypeLabel(row),
    field(row, ["Season"]) || formatDateRange(row),
...
function compareKeyNotes(row) {
  return [
    breakfastLabel(row),
    formatDateRange(row) ? `Validity: ${formatDateRange(row)}` : "",
    field(row, ["Supplements / Blackouts"]) ? `Supplements/blackouts: ${shortText(field(row, ["Supplements / Blackouts"]), 120)}` : "",
    field(row, ["Cancellation Policy"]) ? `Cancellation: ${shortText(field(row, ["Cancellation Policy"]), 100)}` : ""
  ].filter(Boolean);
}

function compareTraceHtml(row) {
  const trace = traceFor(row);
  const rowPart = trace.rowUrl
    ? `<a href="${escapeHtml(trace.rowUrl)}" target="_blank" rel="noopener noreferrer">Sheet row ${escapeHtml(trace.rowNumber)}</a>`
    : `${trace.rowNumber ? `Sheet row ${escapeHtml(trace.rowNumber)}` : "Sheet row not returned"}`;

  return `
    <div class="compare-trace">
      <span>${escapeHtml(trace.recordId || "No Record ID")}</span>
      <span>${rowPart}</span>
      ${trace.sourceContract ? `<span>${escapeHtml(trace.sourceContract)}</span>` : ""}
    </div>
  `;
}
          ...
          ${groups.map(group => {
            ...
            return `
              <article class="compare-card ${isLowest ? "lowest" : ""}">
                ...
                <label class="compare-label">Choose room / season</label>
