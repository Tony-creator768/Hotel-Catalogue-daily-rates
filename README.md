...
The website keeps this data available for auditing through the API and compare panel, but the **View details** modal no longer shows Google Sheet traceability, source rows, or record IDs. This keeps the public-facing details clean while the backend can still be audited when needed.
