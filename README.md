...
Every row returned by the new `Code.gs` API includes a `_trace` object with:
...
The website now displays this in the hotel detail modal and in the compare panel. This makes every rate, room type, blackout note, cancellation term, contact, and policy traceable back to the exact Google Sheet row.
...
- Clear all
- Choose the room/season per hotel from a clearer dropdown
- Highlight the lowest selected rate
- Show breakfast, validity, blackout/cancellation snippets, Record ID, and Sheet Row
...
- Kept traceability in the View details area/source row columns so the data is still auditable without cluttering the cards.
